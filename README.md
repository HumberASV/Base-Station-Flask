...
| `FLASK_DEBUG` | `1` to enable debug mode (optional) |

## Debug Mode

Set `FLASK_DEBUG=1` to enable debug mode. This turns on Flask's interactive debugger and reloader, and sets the Python log level to `DEBUG` so all bridge and ROS2 messages are visible.

```bash
FLASK_DEBUG=1 python app.py
```

Or with docker-compose, add it to the environment section:
...
> [!WARNING]
> Never run with `FLASK_DEBUG=1` in production — the interactive debugger allows arbitrary code execution on the server.
